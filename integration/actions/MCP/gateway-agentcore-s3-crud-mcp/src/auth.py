...
class OAuthTokenValidator:
    ...
    def validate_token(
        ...
        if not access_token or not access_token.strip():
            ...
            return TokenValidationResult(
                is_valid=False, error_message="Access token is required"
            )
        ...
        try:
            ...
            if len(token_parts) != 3:
                ...
                return TokenValidationResult(
                    is_valid=False, error_message="Invalid JWT token format"
                )
            ...
            if not header or not payload:
                ...
                return TokenValidationResult(
                    is_valid=False, error_message="Invalid JWT token encoding"
                )

            # Validate token claims
            validation_result = self._validate_token_claims(
                payload, source_ip, request_id
            )
            ...
            if client_id:
                security_logger_instance.log_authentication_success(
                    client_id=client_id, source_ip=source_ip, request_id=request_id
                )

            return TokenValidationResult(
                is_valid=True, token_claims=payload, scopes=scopes
            )
            ...
            return TokenValidationResult(
                is_valid=False, error_message=f"Token validation error: {str(e)}"
            )

    def _decode_jwt_part(self, encoded_part: str) -> dict[str, Any] | None:
    ...
    def _validate_token_claims(
        ...
        if nbf and current_time < nbf:
            ...
            return TokenValidationResult(
                is_valid=False, error_message="Token is not yet valid"
            )
        ...
        if not iat or iat > current_time:
            ...
            return TokenValidationResult(
                is_valid=False, error_message="Invalid token issued time"
            )
        ...
        if token_use != "access":
            security_logger_instance.log_authentication_failure(
                ...
                error_message="Invalid token use. Expected: access",
                source_ip=source_ip,
                client_id=client_id,
                request_id=request_id,
            )
        ...
        if grant_type != "client_credentials":
            security_logger_instance.log_authentication_failure(
                ...
                error_message="Invalid grant type. Expected: client_credentials",
                source_ip=source_ip,
                client_id=client_id,
                request_id=request_id,
            )
        ...
        missing_scopes = [
            scope for scope in self.required_scopes if scope not in token_scopes
        ]
        if missing_scopes:

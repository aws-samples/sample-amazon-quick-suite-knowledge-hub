...
class Config:
    ...
    SECURITY_LOG_GROUP: str | None = os.environ.get("SECURITY_LOG_GROUP")
    ...
    MAX_OBJECT_SIZE: int = int(
        os.environ.get("MAX_OBJECT_SIZE", "5242880")
    )  # 5MB default
    ...
    def validate_https_configuration(cls) -> None:
        ...
        try:
            test_s3_client = boto3.client(
                "s3", config=BotoCoreConfig(use_ssl=True, signature_version="s3v4")
            )
            ...
            raise RuntimeError(f"Failed to configure S3 client with HTTPS: {e}") from e

        # Test CloudWatch Logs client HTTPS configuration
        try:
            test_logs_client = boto3.client(
                "logs", config=BotoCoreConfig(use_ssl=True, signature_version="v4")
            )
            # Verify the client is configured for HTTPS
            if not test_logs_client._client_config.use_ssl:
                raise RuntimeError(
                    "CloudWatch Logs client is not configured to use HTTPS"
                )
        except Exception as e:
            raise RuntimeError(
                f"Failed to configure CloudWatch Logs client with HTTPS: {e}"
            ) from e

        logging.getLogger(__name__).info(
            "HTTPS/TLS configuration validated successfully"
        )

...
def _check_rate_limit(source_ip: str, request_id: str | None = None) -> bool:
    """
    ...
    if source_ip in _request_counts:
        _request_counts[source_ip]["requests"] = [
            req_time
            for req_time in _request_counts[source_ip]["requests"]
...
def lambda_handler(event: dict[str, Any], context) -> dict[str, Any]:
    ...
    try:
        ...
        user_agent = (
            event.get("requestContext", {}).get("identity", {}).get("userAgent")
        )
...
def handle_mcp_request(event: dict[str, Any], context) -> dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Processing MCP request: tool_name={tool_name}, key={arguments.get('key')}"
        )
        ...
        if not operation:
            return ErrorHandler.handle_validation_error(
                "INVALID_TOOL", f"Unsupported tool: {tool_name}"
            )
        ...
        if not key:
            return ErrorHandler.handle_validation_error(
                "MISSING_KEY", "Object key parameter is required"
            )
        ...
        if operation == "create":
            if not content:
                return ErrorHandler.handle_validation_error(
                    "MISSING_CONTENT",
                    "Content parameter is required for create operation",
                )
            ...
            if not content:
                return ErrorHandler.handle_validation_error(
                    "MISSING_CONTENT",
                    "Content parameter is required for update operation",
                )
            ...
            return ErrorHandler.handle_validation_error(
                "INVALID_OPERATION", f"Unsupported operation: {operation}"
            )
...
def handle_direct_request(event: dict[str, Any], context) -> dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Processing direct request: operation={event.get('operation')}, key={event.get('key')}"
        )
        ...
        if not operation:
            return ErrorHandler.handle_validation_error(
                "MISSING_OPERATION", "Operation parameter is required"
            )

        if not bucket:
            return ErrorHandler.handle_validation_error(
                "MISSING_BUCKET", "Bucket parameter is required"
            )
...
def create_error_response(
    ...
    response = {
        ...
        "body": {"success": False, "error": {"code": error_code, "message": message}},
    }
...
def handle_create_operation(
    bucket: str, key: str, content: str, metadata: dict[str, Any]
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        put_params = {"Bucket": bucket, "Key": key, "Body": content}
        ...
        response = retry_s3_operation(lambda: s3_client.put_object(**put_params))
...
def handle_read_operation(bucket: str, key: str) -> dict[str, Any]:
    ...
    try:
        ...
        # Log successful operation
        logger.info(
            f"Successfully read object: bucket={bucket}, key={key}, size={object_metadata['size']}"
        )
...
def handle_update_operation(
    ...
    try:
        ...
        s3_client = get_s3_client()
        try:
            retry_s3_operation(lambda: s3_client.head_object(Bucket=bucket, Key=key))
        except ClientError as e:
...
def handle_delete_operation(bucket: str, key: str) -> dict[str, Any]:
    ...
    try:
        ...
        retry_s3_operation(lambda: s3_client.delete_object(Bucket=bucket, Key=key))
        ...
        try:
            ...
            logger.error(
                f"Object still exists after delete operation: bucket={bucket}, key={key}"
            )
            return ErrorHandler._create_error_response(
                500, "DELETE_FAILED", "Object deletion was not successful"
            )
        except ClientError as e:
            if e.response["Error"]["Code"] == "NoSuchKey":
        ...
        # Return success response
        return create_success_response(
            {"operation": "delete", "bucket": bucket, "key": key, "deleted": True}
        )

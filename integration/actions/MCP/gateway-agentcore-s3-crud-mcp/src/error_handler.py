...
class ErrorHandler:
    ...
    def handle_s3_client_error(
        e: ClientError, operation: str, bucket: str = None, key: str = None
    ) -> dict[str, Any]:
        """
        ...
        logger.error(
            f"S3 ClientError in {operation} operation: {error_code} - {error_message}"
        )
        ...
        error_mappings = {
            "NoSuchKey": {
                ...
                "message": f"The specified object does not exist: {key}"
                if key
            ...
            "InvalidBucketName": {
                ...
                "message": f"Invalid bucket name: {bucket}"
                if bucket
                else "Invalid bucket name",
            },
            "BucketNotEmpty": {
                "status_code": 409,
        ...
        mapped_error = error_mappings.get(
            error_code,
            {
                "status_code": 500,
                "error_code": "S3_ERROR",
    ...
    def handle_validation_error(
        error_type: str, message: str, details: dict[str, Any] = None
    ) -> dict[str, Any]:
        """
        ...
        return ErrorHandler._create_error_response(400, error_type, message, details)
    ...
    def handle_credentials_error(operation: str) -> dict[str, Any]:
        ...
        return ErrorHandler._create_error_response(
            500, "CREDENTIALS_ERROR", "AWS credentials not available"
        )
    ...
    def handle_network_error(e: Exception, operation: str) -> dict[str, Any]:
        ...
        return ErrorHandler._create_error_response(
            503, "NETWORK_ERROR", "Network connectivity issue occurred"
        )

...
class SecurityLogger:
    ...
    def log_suspicious_activity(
        ...
        event = SecurityEvent(
            ...
            additional_data={"activity_type": activity_type, **(additional_data or {})},
            request_id=request_id,
        )
        self.log_security_event(event)
    ...
    def _send_to_security_log_group(self, log_entry: dict[str, Any]) -> None:
        ...
        try:
            log_stream_name = (
                f"security-events-{int(time.time() // 3600)}"  # Hourly streams
            )

            # Create log stream if it doesn't exist
            try:
                self.cloudwatch_client.create_log_stream(
                    logGroupName=self.security_log_group, logStreamName=log_stream_name
                )

...
@given(
    invalid_audience=st.sampled_from(
        ["wrong-audience", "invalid-aud", "other-service"]
    ),
    client_id=st.text(
        min_size=5,
        max_size=30,
...
@given(
    ...
    invalid_scopes=st.lists(
        st.sampled_from(["s3:read", "gateway:read", "invalid:scope"]),
        min_size=0,
        max_size=2,
    ),
    invalid_grant_type=st.sampled_from(
...
def test_property_authentication_failure_handling(
    client_id,
    exp_offset,
    invalid_audience,
    invalid_scopes,
    ...
    **Feature: bedrock-agent-gateway, Property 2: Service authentication failure handling**
    ...
    for token in test_cases:
        ...
        assert result.error_message is not None, (
            "Error message should be provided for invalid token"
        )
        ...
        assert result.token_claims is None or not result.is_valid, (
            "Invalid tokens should not return claims"
        )
        assert result.scopes is None or not result.is_valid, (
            "Invalid tokens should not return scopes"
        )

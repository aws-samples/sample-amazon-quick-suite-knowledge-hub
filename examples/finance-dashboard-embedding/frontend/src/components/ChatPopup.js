  ...
  return (
    <div className="qs-chat-panel">
      ...
      <div ref={containerRef} className="qs-chat-panel-body">
        ...
        {!chatEmbedUrl && !loading && !error && (
          <div className="chat-loading">
            <span>Waiting for dashboard...</span>
          </div>
        )}
      </div>
    </div>
...
export default function ChatPopup() {
  ...
  const getFreshToken = useCallback(
    async (currentToken, currentRefresh) => {
      ...
      exchangeCodeForTokens(code)
        .then((tokens) => {
          ...
          if (tokens.refresh_token)
            sessionStorage.setItem("qs_refresh_token", tokens.refresh_token);
    ...
    const embed = async () => {
      ...
      try {
        ...
        try {
          ...
          throw new Error(
            "Unable to reach embedding API — CORS may not be enabled for this origin",
          );
        }
        const data = await res.json();
        if (data.status !== "SUCCESS" || !data.embedUrl)
          throw new Error(data.status || "No embed URL");
    ...
    return (
      <div className="chat-fab-wrapper">
        <button
          ...
          title="Sign in to access Finance Assistant"
        >
  ...
  return (
    <>
      <div className="chat-auth-bar">
        ...
        <button onClick={handleSignOut} className="chat-signout-btn">
          Sign Out
        </button>
      </div>
      <div className="chat-fab-wrapper">
      ...
      {isOpen && (
        <div className="qs-dashboard-overlay">
          <div className="qs-dashboard-header">
            ...
            <small>
              Click the chat button below to ask the AI assistant questions about this data
            </small>
            <button className="qs-close-btn" onClick={() => setIsOpen(false)}>
              ✕ Close
            </button>
          </div>

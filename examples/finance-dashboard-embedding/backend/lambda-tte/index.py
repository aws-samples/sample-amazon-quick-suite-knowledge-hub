...
def lambda_handler(event, context):
    logger.info(f"IDC_APP_CLIENT_ID: {IDC_APP_CLIENT_ID}")
        ...
        if len(post_string) > 0:
            ...
            if "idToken" in post_json:
                ...
                try:
                    ...
                    logger.info(
                        "Cognito token claims: iss={}, aud={}, email={}, token_use={}".format(
                            ...
                            cognito_claims.get("aud"),
                            cognito_claims.get("email"),
                            cognito_claims.get("token_use"),
                        )
                    )
                ...
                try:
                    ...
                    logger.error(f"OIDC token error: {str(e)}")
                    ...
                    result = {
                        "embedUrl": "",
                        "status": f"Exception: {type(e).__name__}: {error_message} (Code: {error_code})",
                    }
                else:
                    ...
                    try:
                        user_creds = sts_client.assume_role(
                            RoleArn=ROLE_ARN,
                            RoleSessionName="quicksuite_session_"
                            + str(x.strftime("%f")),
                        ...
                        quicksuite = boto3.client(
                            ...
                            aws_secret_access_key=user_creds["Credentials"][
                                "SecretAccessKey"
                            ],
                        ...
                        try:
                            logger.info(
                                f"=== GENERATING EMBED URL (type={embed_type}) ==="
                            )

                            if embed_type == "both" and DASHBOARD_ID:
                                dash_response = (
                                    quicksuite.generate_embed_url_for_registered_user(
                                ...
                                logger.info(
                                    "Dashboard embed URL generated successfully"
                                )
                                result = {
                                ...
                                logger.info(
                                    "QuickChat embed URL generated successfully"
                                )
                                result = {
                                    "embedUrl": response["EmbedUrl"],
                                    "status": "SUCCESS",
                            ...
                            logger.error(f"QuickSuite embed error: {str(e)}")
                            result = {
                                "embedUrl": "",
            ...
            result = {
                "embedUrl": "",
                "status": "ERROR: JSON document body not provided",
            }

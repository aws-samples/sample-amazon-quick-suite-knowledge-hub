    ...
    for day in range(DAYS):
        ...
        for app in APPLICATIONS:
            for model in MODELS:
                ...
                guardrail_evaluations = int(
                    daily_invocations * random.uniform(0.3, 0.7)
                )
                guardrail_cost = (
                    guardrail_evaluations * 0.00075
                )  # $0.75 per 1K evaluations

                # Provisioned throughput (some apps use it)
                if (
                    app in ["customer-support-chatbot", "content-generation-api"]
...
def main():
    ...
    print("Generating Bedrock cost and usage data...")
    print(f"Days: {DAYS}")

...
def create_quicksight_resources(
    ...
    principal_arn = (
        f"arn:aws:quicksight:{region}:{account_id}:user/default/{quicksight_user}"
    )
    ...
    try:
        ...
        ds_desc = quicksight.describe_data_source(
            AwsAccountId=account_id, DataSourceId="genai-ops-athena-source"
        )
...
if __name__ == "__main__":
    ...
    print(
        "If this is incorrect, run: python3 create_quicksight_resources.py 'Admin/your-username'"
    )

    create_quicksight_resources(account_id=account_id, quicksight_user=quicksight_user)

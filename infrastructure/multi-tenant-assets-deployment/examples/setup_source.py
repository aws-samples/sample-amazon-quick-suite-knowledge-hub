...
def setup_knowledge_base(qs, s3, args, space_id, cfg, principal):
    ...
    # 1. Bucket + policy
    print("  Knowledge base — S3 bucket:")
    ...
    ds_params = {
        ...
        "DataSourceParameters": {
            "S3KnowledgeBaseParameters": {"BucketUrl": f"s3://{bucket_name}"}
        },
    }
    ...
    if principal:
        ...
        create_params["Permissions"] = [
            {"Principal": principal, "Actions": KB_PERMISSIONS}
        ]
    ...
    if kb_already:
        ...
        try:
            ...
            print(
                f"  ✓ Knowledge base created: {kb_id} (status: {resp.get('CreationStatus')})"
            )
            if principal:
                print(f"  ✓ KB permissions granted ({len(KB_PERMISSIONS)} actions)")
            kb_exists = True
    ...
    if kb_exists:
        try:
            ...
            print("  ✓ Knowledge base linked to space")
        except Exception as e:
            print(f"  ⚠️  KB link: {e}")
        return kb_arn
    return None
...
def main():
    parser = argparse.ArgumentParser(
        description="Setup 2 spaces + 2 agents (+ connector + KB) in source"
    )
    parser.add_argument(
        "--account-id",
        default="",
    ...
    parser.add_argument(
        "--qs-service-role",
        default=DEFAULTS["qs_service_role"],
    ...
    if not args.account_id:
        args.account_id = session.client(
    ...
    principal = None
    try:
        users = qs.list_users(AwsAccountId=args.account_id, Namespace="default").get(
            "UserList", []
        )
    ...
    if principal:
        try:
            qs.update_action_connector_permissions(
                ...
                ActionConnectorId=args.connector_id,
                GrantPermissions=[
                    {"Principal": principal, "Actions": CONNECTOR_PERMISSIONS}
                ],
            )
            print("  ✓ Connector permissions granted")
        except Exception as e:
            print(f"  ⚠️  {e}")
    ...
    for space_id, cfg in SPACES.items():
        space_arn = (
            f"arn:aws:quicksight:{args.region}:{args.account_id}:space/{space_id}"
        )
        print(f"\n{'=' * 60}")
        ...
        if principal:
            try:
                qs.update_space_permissions(
                    ...
                    SpaceId=space_id,
                    GrantPermissions=[
                        {"Principal": principal, "Actions": SPACE_PERMISSIONS}
                    ],
                )
                print("  ✓ Space permissions granted")
            except Exception as e:
        ...
        if cfg["has_connector"]:
            try:
                ...
                print("  ✓ Slack connector linked to space")
            except Exception as e:
        ...
        try:
            ...
            try:
                cur = qs.describe_agent(AwsAccountId=args.account_id, AgentId=agent_id)[
                    "Agent"
                ]
        ...
        if principal:
            try:
                ...
                qs.update_agent_permissions(
                    ...
                    AgentId=agent_id,
                    GrantPermissions=[
                        {"Principal": principal, "Actions": AGENT_PERMISSIONS}
                    ],
                )
                print("  ✓ Agent permissions granted")
            except Exception as e:
    ...
    print(
        f"  KB bucket: {kb_bucket_name(args.env, args.account_id)} (linked to analytics-space)"
    )
